...
class Jamworks:
    ...
    def getContentsFileInfo(self, node_id):
        ...
        node_data_req = requests.get(url = info_url, headers = self.getCorrectToken(), timeout=20)
    ...
    def contentsDownloadFile(self, node_id, filename):
        ...
        with requests.get(
            url=download_url, headers=self.getCorrectToken(), stream=True, timeout=600
        ) as r:
    ...
    def contentsList(self, node_id):
        ...
        response = requests.get(url=requestUrl, headers=self.getCorrectToken(), timeout=20)
        data = response.json()
    ...
    def contentsUploadFile(self, tenant_id, parent_nodeid, filename):
        ...
        response = requests.post(
            url = upload_url, headers=self.getCorrectToken(), files=files, data=data, timeout=600
        )
        data = response.json()
        if 'message' in data and len(data) == 1:
            raise Exception(data['message'])
    ...
    def contentsInactivateRendition(self, relationship_type, node_type, node_id):
        inactivate_url = (
            self.content_url
            + "/rendition/inactivate/"
            + str(node_id)
            + "?filters[relationship_type]="
            + relationship_type
            + "&filters[active]=1&filters[node_type]="
            + node_type
        )
        response = requests.delete(url=inactivate_url, headers=self.getCorrectToken(), timeout=20)
        data = response.json()
        if 'message' in data and len(data) == 1:
            raise Exception(data['message'])
        return data

    def contentsUploadRendition(
        self,
        relationship_type,
        node_type,
        node_id,
        filename,
        item_index,
        custom_destination_suffix=None,
    ):
        ...
        files = {"file": open(filename, "rb")}
        response = requests.post(
            url=upload_url, headers=self.getCorrectToken(), files=files, data=data, timeout=20
        )
        data = response.json()
        if 'message' in data and len(data) == 1:
            raise Exception(data['message'])
    ...
    def contentsDownloadRenditionFile(self, node_id, filename):
        ...
        with requests.get(
            url=download_url, headers=self.getCorrectToken(), stream=True, timeout=20
        ) as r:
            with open(filename, "wb") as f:
                for chunk in r.iter_content(chunk_size=16 * 1024):
                    f.write(chunk)
    ...
    def contentsGetPreviewUrls(self, node_id):
        ...
        response = requests.get(url=download_url, headers=self.getCorrectToken(), timeout=20)
        data = response.json()
        if 'message' in data and len(data) == 1:
            raise Exception(data['message'])
        return data
    ...
    def contentsExportSheet(self, nodeid, sheetName="", format="json", skip=0):
        ...
        response = requests.get(url=exportUrl, headers=self.getCorrectToken(), timeout=20)
        data = response.json()
        if 'message' in data and len(data) == 1:
            raise Exception(data['message'])
        return data
    ...
    def contentsUpdateNode(self, node_id, data):
        ...
        response = requests.put(
            url=update_url, headers=self.getCorrectToken(), json=data, timeout=20
        )
